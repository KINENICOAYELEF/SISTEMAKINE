...
function evaluarROM(inputId, valorMin, valorModerado, valorNormal) {
  ...
  // Si no se proporcionaron valores, obtenerlos automáticamente
  let valoresNormativos = {
    valorMin: valorMin,
    valorModerado: valorModerado,
    valorNormal: valorNormal
  };
  
  if (!valorNormal || valorNormal <= 0) {
    const region = inputId.split('_')[0]; // Ej: cervical, hombro, etc.
    const movimiento = extraerMovimientoDesdeId(inputId);
    
    valoresNormativos.valorNormal = obtenerValorNormativo(region, movimiento);
    valoresNormativos.valorModerado = valoresNormativos.valorNormal * 0.6;
    valoresNormativos.valorMin = 0;
  }
  ...
  if (valor < valoresNormativos.valorModerado) {
    estado = "Disminuido";
    colorClase = "bg-danger text-white";
  } else if (valor < valoresNormativos.valorNormal) {
    estado = "Limitado";
    colorClase = "bg-warning";
  } else {
    estado = "Normal";
...
// Función global para forzar todos los cálculos
function forzarCalculosCompletos() {
  console.log("=== FORZANDO TODOS LOS CÁLCULOS DE HOMBRO ===");
  
  // 1. Recopilar y mostrar todos los valores ingresados
  const todosLosInputs = document.querySelectorAll('input[id^="hombro_"]');
  ...
  // 2. Forzar evaluación de ROM para cada input activo
  const inputsActivos = document.querySelectorAll('input[id^="hombro_"][id$="_activo"]');
  console.log("Forzando evaluación de " + inputsActivos.length + " inputs activos");
  
  inputsActivos.forEach(input => {
    if (input.value) {
      const valor = parseFloat(input.value);
      const movimiento = input.id.split('_')[1]; // Extraer movimiento del ID
      
      // Obtener valores normativos
      let valorNormal = 0;
      switch(movimiento) {
        case "flexion": valorNormal = 180; break;
        case "extension": valorNormal = 50; break;
        case "abduccion": valorNormal = 180; break;
        case "aduccion": valorNormal = 30; break;
        case "rot": 
          if (input.id.includes("_int")) valorNormal = 70;
          else if (input.id.includes("_ext")) valorNormal = 90;
          break;
      }
      
      const valorModerado = valorNormal * 0.6;
      console.log(`Evaluando ${input.id} con valor=${valor}, valorNormal=${valorNormal}`);
      
      // Actualizar estado visual
      const estadoElement = document.getElementById(input.id + "_estado");
      if (estadoElement) {
        let estado = "Normal";
  ...
  // 3. Calcular déficit funcional directamente
  console.log("Calculando déficit funcional");
  let totalDeficit = 0;
  let movimientosEvaluados = 0;
  
  // Valores normativos específicos para hombro
  const valoresNormativos = {
    "flexion": 180,
    "extension": 50,
    "abduccion": 180,
  ...
  inputsActivos.forEach(input => {
    if (input.value) {
      ...
      // Extraer el movimiento correctamente
      if (input.id.includes("flexion")) movimiento = "flexion";
      else if (input.id.includes("extension")) movimiento = "extension";
      else if (input.id.includes("abduccion")) movimiento = "abduccion";
      else if (input.id.includes("aduccion")) movimiento = "aduccion";
      else if (input.id.includes("rot_int")) movimiento = "rot_int";
      else if (input.id.includes("rot_ext")) movimiento = "rot_ext";
      
      const valorNormativo = valoresNormativos[movimiento] || 0;
      
      if (valorNormativo > 0) {
        const deficitPorcentaje = Math.max(0, Math.min(100, 100 - (valor / valorNormativo * 100)));
        console.log(`${movimiento}: valor=${valor}, normativo=${valorNormativo}, déficit=${deficitPorcentaje.toFixed(1)}%`);
        totalDeficit += deficitPorcentaje;
        movimientosEvaluados++;
      }
    }
  ...
  if (movimientosEvaluados > 0) {
    const deficitPromedio = totalDeficit / movimientosEvaluados;
    console.log(`Déficit promedio: ${deficitPromedio.toFixed(1)}%`);
  ...
  // 4. Generar interpretación directamente
  console.log("Generando interpretación y recomendaciones");
  ...
  // Contar movimientos con limitación
  const movimientosDisminuidos = document.querySelectorAll('.bg-danger:not([id$="_dolor"])').length;
  const movimientosLimitados = document.querySelectorAll('.bg-warning:not([id$="_dolor"])').length;
  ...
  // 5. Generar recomendaciones directamente
  let recomendaciones = "<ul>";
  ...
  return "Cálculos completados correctamente";
}

// Añadir botón al DOM directamente (forma alternativa)
function crearBotonCalculoManual() {
  // Buscar un contenedor adecuado para el botón
  let contenedor = document.querySelector('.calculadora-deficit-hombro');
  if (!contenedor) {
    // Si no se encuentra el contenedor específico, buscar uno más genérico
    contenedor = document.querySelector('[id*="hombro_deficit"]');
    if (!contenedor) {
      // Como último recurso, buscar cualquier div relacionado con hombro
      contenedor = document.querySelector('div[id*="hombro"]');
    }
  }
  
  if (contenedor) {
    // Crear botón si no existe ya
    if (!document.getElementById('btn_calcular_hombro')) {
      const boton = document.createElement('button');
      boton.id = 'btn_calcular_hombro';
      boton.className = 'btn btn-primary mt-2 w-100';
      boton.textContent = 'CALCULAR DÉFICIT HOMBRO Y ACTUALIZAR';
      ...
      boton.onclick = function() {
        forzarCalculosCompletos();
        alert('¡Cálculos e interpretaciones actualizados!');
      };
      
      // Añadir el botón
      contenedor.appendChild(boton);
      console.log("Botón de cálculo manual creado exitosamente");
    }
  } else {
    console.error("No se encontró ningún contenedor para añadir el botón");
    
    // Como alternativa, crear un botón flotante
    const botonFlotante = document.createElement('button');
    botonFlotante.id = 'btn_calcular_hombro_flotante';
    botonFlotante.className = 'btn btn-danger';
    botonFlotante.textContent = 'CALCULAR HOMBRO';
    ...
    botonFlotante.style.padding = '15px';
    ...
    console.log("Botón flotante de emergencia creado");
  }
}

// Añadir botón cuando el DOM esté cargado
setTimeout(crearBotonCalculoManual, 1500);

// Crear función global para uso desde consola en caso de emergencia
window.actualizarHombro = forzarCalculosCompletos;
// ============= FIN DE SOLUCIÓN DE EMERGENCIA =============
});
